...
static const char *c128rom_active_kernal_resource_name(int machine_type)
{
    switch (machine_type) {
        case C128_MACHINE_INT:
            return "KernalIntName";
        case C128_MACHINE_FINNISH:
            return "KernalFIName";
        case C128_MACHINE_FRENCH:
            return "KernalFRName";
...
static const char *c128rom_active_chargen_resource_name(int machine_type)
{
    switch (machine_type) {
        case C128_MACHINE_INT:
            return "ChargenIntName";
        case C128_MACHINE_ITALIAN:
            return "ChargenITName";
        case C128_MACHINE_FRENCH:
            return "ChargenFRName";
        case C128_MACHINE_GERMAN:
            return "ChargenDEName";
        case C128_MACHINE_FINNISH:
            return "ChargenFIName";
        case C128_MACHINE_SWEDISH:
            return "ChargenSEName";
        case C128_MACHINE_SWISS:
            return "ChargenCHName";
        case C128_MACHINE_NORWEGIAN:
            return "ChargenNOName";
        default:
            return NULL;
    }
}
...
{
    int machine_type;
    const char *kernal_resname;
    const char *chargen_resname;
    ...
    resources_get_int("MachineType", &machine_type);
    kernal_resname = c128rom_active_kernal_resource_name(machine_type);
    chargen_resname = c128rom_active_chargen_resource_name(machine_type);
    if (kernal_resname == NULL || chargen_resname == NULL) {
        log_error(c128rom_log, "Unknown machine type %i.", machine_type);
        return -1;
    }

    if (c128rom_check_res_file(kernal_resname) < 0) {
        return -1;
    }

    c128rom_kernal_setup();

    if (c128rom_check_res_file(chargen_resname) < 0) {
        return -1;
    }
