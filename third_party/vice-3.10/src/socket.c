...
static int vice_network_close_raw_socket(SOCKET sockfd)
{
#ifdef RASPI_COMPILE
    return emux_network_socket_close(sockfd);
#else
        ...
        const vice_network_socket_address_t * server_address)
{
    ...
    if (error) {
        if (sockfd != INVALID_SOCKET) {
            vice_network_close_raw_socket(sockfd);
        }
    ...
    do {
        ...
        if (address_string) {
...

            {
                ...
                if (inet_pton(AF_INET, address_part,
                              &socket_address->address.ipv4.sin_addr) == 1) {
                    resolved = 1;
                } else if (emux_network_is_ready()) {
                    memset(&hints, 0, sizeof hints);
                    hints.ai_family = AF_INET;
                    hints.ai_socktype = SOCK_STREAM;
                    hints.ai_protocol = IPPROTO_TCP;
                    if (getaddrinfo(address_part, NULL, &hints, &result) == 0 &&
                        result != NULL && result->ai_addr != NULL &&
                        result->ai_addrlen >= sizeof(struct sockaddr_in)) {
                        struct sockaddr_in *ipv4 =
                            (struct sockaddr_in *)result->ai_addr;
                        socket_address->address.ipv4.sin_addr = ipv4->sin_addr;
                        resolved = 1;
                    }
                    if (result != NULL) {
                        freeaddrinfo(result);
                    ...
                    log_message(LOG_DEFAULT,
                                "network not ready; cannot resolve `%s'.",
                                address_part);
                }

                if (!resolved)
#else
                struct hostent * host_entry = gethostbyname(address_part);
...
int vice_network_socket_close(vice_network_socket_t * sockfd)
{
    ...
    if (sockfd) {
        ...
        error = vice_network_close_raw_socket(localsockfd);
    }
