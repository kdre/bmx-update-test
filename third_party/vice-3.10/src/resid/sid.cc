...
void SID::debugoutput(void)
{
    ...
    if (recording == -1) {
        ...
        recording = 0;
        myFile = fopen("resid.raw", "wb");
        lastn = n;
        printf("reSID: waiting for output to change...\n");
    } else if ((recording == 0) && (lastn != n)) {
        /* start recording when the reSID output changes */
        recording = 1;
        printf("reSID: starting recording...\n");
    }
    /* write 16bit little endian signed data */
    if (recording && myFile != NULL) {
        fputc(n & 0xff, myFile);
        fputc((n >> 8) & 0xff, myFile);
    }
}
...
void SID::enable_raw_debug_output(bool enable)
{
    ...
    if (enable) {
        printf("reSID: raw output enabled.\n");
    }
}
