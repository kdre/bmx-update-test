...
static unsigned long acia_dbg_clk(void)
{
    return (unsigned long)(myclk & 0xffffffffUL);
}
#endif

#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
static unsigned int acia_dbg_store_count;
static unsigned int acia_dbg_read_count;
static unsigned int acia_dbg_tx_count;
static unsigned int acia_dbg_rx_count;
static unsigned int acia_dbg_rx_poll_count;

static int acia_dbg_should_log(unsigned int count)
{
    return count <= 64 || (count % 256) == 0;
}

static int acia_dbg_should_log_store(uint16_t addr, int reg_mask, uint8_t byte,
                                     unsigned int count)
{
    static uint8_t last_value[8];
    static uint8_t have_value[8];
    unsigned int reg = addr & reg_mask;

    if (reg == ACIA_DR) {
        return acia_dbg_should_log(count);
    }

    if (reg < sizeof last_value &&
        (!have_value[reg] || last_value[reg] != byte)) {
        last_value[reg] = byte;
        have_value[reg] = 1;
        return 1;
    }
...
static char acia_dbg_chr(uint8_t b)
{
    return (b >= 0x20 && b <= 0x7e) ? (char)b : '.';
}

static const char *acia_dbg_reg_name(uint16_t addr, int reg_mask)
{
    switch (addr & reg_mask) {
        case ACIA_DR:
            return "DR";
        case ACIA_SR:
            return "SR";
...
#ifdef BMC64_DEBUG_PROFILE
static void acia_dbg_state(const char *tag)
{
    BMC64_ACIA_DEBUG("%s fd %d dev %d mode %d irq_type %d cmd 0x%02x ctrl 0x%02x "
                     "status 0x%02x ectrl 0x%02x ticks %d bps %.2f txstate %d "
                     "rxactive %d txactive %d hs 0x%02x clk %lu",
                     tag, acia.fd, acia.device, acia.mode, acia.irq_type,
                     acia.cmd, acia.ctrl, acia.status, acia.ectrl,
                     acia.ticks, get_acia_bps(), acia.in_tx,
                     acia.alarm_active_rx, acia.alarm_active_tx,
                     acia.rs232_status_lines, acia_dbg_clk());
}
#endif
...
#ifdef BMC64_DEBUG_PROFILE
    {
        static int dbg_last_ticks = -1;
        static uint8_t dbg_last_ctrl = 0xff;
        static uint8_t dbg_last_cmd = 0xff;
        static uint8_t dbg_last_ectrl = 0xff;
        if (acia.ticks != dbg_last_ticks || acia.ctrl != dbg_last_ctrl ||
            acia.cmd != dbg_last_cmd || acia.ectrl != dbg_last_ectrl) {
            BMC64_ACIA_DEBUG("ticks bps %.2f ticks %d datamask 0x%02x cmd 0x%02x "
                             "ctrl 0x%02x ectrl 0x%02x mode %d clk %lu",
                             get_acia_bps(), acia.ticks, acia.datamask,
                             acia.cmd, acia.ctrl, acia.ectrl, acia.mode,
                             acia_dbg_clk());
            dbg_last_ticks = acia.ticks;
            dbg_last_ctrl = acia.ctrl;
            dbg_last_cmd = acia.cmd;
            dbg_last_ectrl = acia.ectrl;
        }
    }
#endif
...
static int acia_get_status(void)
{
...
#ifdef BMC64_DEBUG_PROFILE
    {
        ...
        if (acia.status != dbg_oldstatus) {
            BMC64_ACIA_DEBUG("status fd %d modem 0x%02x acia_status 0x%02x "
                             "dcd_bit %d dsr_bit %d clk %lu",
                             acia.fd, modem_status, acia.status,
                             (acia.status & ACIA_SR_BITS_DCD) ? 1 : 0,
                             (acia.status & ACIA_SR_BITS_DSR) ? 1 : 0,
                             acia_dbg_clk());
            dbg_oldstatus = acia.status;
        }
    }
#endif
...
static void acia_set_handshake_lines(void)
{
    ...
    {
        static int dbg_oldstatus = -1;
        if (acia.rs232_status_lines != dbg_oldstatus) {
            BMC64_ACIA_DEBUG("handshake fd %d lines 0x%02x dtr %d rts %d cmd 0x%02x "
                             "rxactive %d txactive %d clk %lu",
                             acia.fd, acia.rs232_status_lines,
                             (acia.rs232_status_lines & RS232_HSO_DTR) ? 1 : 0,
                             (acia.rs232_status_lines & RS232_HSO_RTS) ? 1 : 0,
                             acia.cmd, acia.alarm_active_rx,
                             acia.alarm_active_tx, acia_dbg_clk());
            dbg_oldstatus = acia.rs232_status_lines;
        }
    }
#endif
...
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
    acia_dbg_store_count++;
    if (acia_dbg_should_log_store(addr, acia_register_size, byte,
                                  acia_dbg_store_count)) {
        BMC64_ACIA_TRACE("store #%u addr 0x%04x reg %s byte 0x%02x '%c' "
                         "fd %d cmd 0x%02x ctrl 0x%02x status 0x%02x txstate %d clk %lu",
                         acia_dbg_store_count, addr,
                         acia_dbg_reg_name(addr, acia_register_size),
                         byte, acia_dbg_chr(byte), acia.fd, acia.cmd,
                         acia.ctrl, acia.status, acia.in_tx, acia_dbg_clk());
    }
#endif
    ...
    switch (addr & acia_register_size) {
        ...
        case ACIA_CMD:
            ...
            if ((acia.cmd & ACIA_CMD_BITS_DTR_ENABLE_RECV_AND_IRQ) && (acia.fd < 0)) {
                acia.fd = rs232drv_open(acia.device);
#ifdef BMC64_DEBUG_PROFILE
                BMC64_ACIA_EVENT("open from CMD fd %d device %d cmd 0x%02x ip232 %d clk %lu",
                                  acia.fd, acia.device, acia.cmd,
                                  rs232_useip232[acia.device],
                                  acia_dbg_clk());
#endif
                /* enable RX alarm */
                acia.alarm_active_rx = 1;
            ...
            if ((acia.fd >= 0) && !(acia.cmd & ACIA_CMD_BITS_DTR_ENABLE_RECV_AND_IRQ) && !rs232_useip232[acia.device]) {
#ifdef BMC64_DEBUG_PROFILE
                BMC64_ACIA_EVENT("close from CMD fd %d device %d cmd 0x%02x clk %lu",
                                  acia.fd, acia.device, acia.cmd,
                                  acia_dbg_clk());
#endif
                rs232drv_close(acia.fd);
...
static uint8_t myacia_read_(uint16_t addr)
{
    ...
    switch (addr & acia_register_size) {
...
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
            acia_dbg_read_count++;
            if (acia_dbg_should_log(acia_dbg_read_count)) {
                BMC64_ACIA_TRACE("read #%u addr 0x%04x reg DR byte 0x%02x '%c' "
                                 "status_before 0x%02x fd %d clk %lu",
                                 acia_dbg_read_count, addr, acia.rxdata,
                                 acia_dbg_chr(acia.rxdata), acia.status,
                                 acia.fd, acia_dbg_clk());
            }
#endif
...
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
                {
                    static uint8_t dbg_last_sr = 0xff;
                    acia_dbg_read_count++;
                    if (c != dbg_last_sr || acia_dbg_should_log(acia_dbg_read_count)) {
                        BMC64_ACIA_TRACE("read #%u addr 0x%04x reg SR byte 0x%02x "
                                         "irq %d fd %d clk %lu",
                                         acia_dbg_read_count, addr, c,
                                         acia.irq, acia.fd, acia_dbg_clk());
                        dbg_last_sr = c;
                    }
                }
#endif
        ...
        case ACIA_CTRL:
            ...
            acia_dbg_read_count++;
            if (acia_dbg_should_log(acia_dbg_read_count)) {
                BMC64_ACIA_TRACE("read #%u addr 0x%04x reg CTRL byte 0x%02x clk %lu",
                                 acia_dbg_read_count, addr, acia.ctrl,
                                 acia_dbg_clk());
            }
#endif
        ...
        case ACIA_CMD:
            ...
            acia_dbg_read_count++;
            if (acia_dbg_should_log(acia_dbg_read_count)) {
                BMC64_ACIA_TRACE("read #%u addr 0x%04x reg CMD byte 0x%02x clk %lu",
                                 acia_dbg_read_count, addr, acia.cmd,
                                 acia_dbg_clk());
            }
#endif
...
static void int_acia_tx(CLOCK offset, void *data)
{
    ...
    if ((acia.in_tx == ACIA_TX_STATE_DR_WRITTEN) && (acia.fd >= 0)) {
        uint8_t out_byte = acia.txdata & acia.datamask;
        enum rs232handshake_in modem_status = rs232drv_get_status(acia.fd);
        if (!(modem_status & RS232_HSI_CTS)) {
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
            BMC64_ACIA_TRACE("tx wait cts fd %d dev %d byte 0x%02x status 0x%02x "
                             "modem 0x%02x clk %lu",
                             acia.fd, acia.device, out_byte, acia.status,
                             modem_status, acia_dbg_clk());
#endif
            acia.alarm_clk_tx = myclk + acia.ticks;
            alarm_set(acia.alarm_tx, acia.alarm_clk_tx);
            acia.alarm_active_tx = 1;
...
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
        acia_dbg_tx_count++;
        if (acia_dbg_should_log(acia_dbg_tx_count)) {
            BMC64_ACIA_TRACE("tx #%u fd %d dev %d byte 0x%02x '%c' txstate %d "
                             "ticks %d status 0x%02x clk %lu",
                             acia_dbg_tx_count, acia.fd, acia.device,
                             out_byte, acia_dbg_chr(out_byte), acia.in_tx,
                             acia.ticks, acia.status, acia_dbg_clk());
        }
#endif
        if (rs232drv_putc(acia.fd, out_byte) < 0) {
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
            BMC64_ACIA_TRACE("tx retry fd %d dev %d byte 0x%02x status 0x%02x "
                             "clk %lu",
                             acia.fd, acia.device, out_byte, acia.status,
                             acia_dbg_clk());
#endif
...
static void int_acia_rx(CLOCK offset, void *data)
{
    ...
    do {
        ...
        /*
         * Do not pull another byte from the host-side RS232 driver while the
         * emulated ACIA receive register is still full. Network sockets can
         * deliver banners/prompts in bursts; reading here would otherwise drop
         * those bytes as ACIA overrun before terminal software has a chance to
        ...
        {
            int rx_ret = rs232drv_getc(acia.fd, &received_byte);
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
            acia_dbg_rx_poll_count++;
            if (rx_ret || acia_dbg_rx_poll_count <= 16 || (acia_dbg_rx_poll_count % 1024) == 0) {
                BMC64_ACIA_TRACE("rxpoll #%u fd %d dev %d ret %d status 0x%02x "
                                 "rxactive %d ticks %d clk %lu",
                                 acia_dbg_rx_poll_count, acia.fd, acia.device,
                                 rx_ret, acia.status, acia.alarm_active_rx,
                                 acia.ticks, acia_dbg_clk());
            }
#endif
            if (!rx_ret) {
                break;
...
#if defined(BMC64_DEBUG_PROFILE) && BMC64_ACIA_LOG_LEVEL >= BMC64_LOG_TRACE
        acia_dbg_rx_count++;
        if (acia_dbg_should_log(acia_dbg_rx_count)) {
            BMC64_ACIA_TRACE("rx #%u fd %d dev %d byte 0x%02x '%c' masked 0x%02x "
                             "status_before 0x%02x clk %lu",
                             acia_dbg_rx_count, acia.fd, acia.device,
                             received_byte, acia_dbg_chr(received_byte),
                             received_byte & acia.datamask, acia.status,
                             acia_dbg_clk());
        }
#endif
