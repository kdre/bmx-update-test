...
#define BMX_HAYES_DEFAULT_DIAL_TARGET "bmxbbs.de:6510"
...
int mdm_connect(modem_config *cfg) {
  ...
  if(cfg->conn_type == MDM_CONN_NONE) {
    rc = line_connect(&cfg->line_data, cfg->dialno);
    if(rc == 0) {
      cfg->conn_type = MDM_CONN_OUTGOING;
      mdm_set_control_lines(cfg);
      mdm_print_speed(cfg);
    } else if(rc != LINE_CONNECT_PENDING) {
      mdm_send_response(MDM_RESP_NO_CARRIER, cfg);
      usleep(cfg->disconnect_delay * 1000);
    }
  }
    ...
    switch(cmd) {
      ...
      case 'D':
        if(end > start) {
          ...
          strncpy(cfg->dialno, BMX_HAYES_DEFAULT_DIAL_TARGET,
                  sizeof(cfg->dialno) - 1);
          cfg->dialno[sizeof(cfg->dialno) - 1] = '\0';
          strncpy(cfg->last_dialno, cfg->dialno, sizeof(cfg->last_dialno) - 1);
          cfg->last_dialno[sizeof(cfg->last_dialno) - 1] = '\0';
          cfg->dial_type = num == 'P' ? 'P' : 'T';
          cfg->last_dial_type = cfg->dial_type;
          cfg->memory_dial = FALSE;
        }
#ifdef BMC64_DEBUG_PROFILE
        printf("tcpserdbg: dial parsed type %c number '%s'\r\n",
               cfg->dial_type ? cfg->dial_type : '-', cfg->dialno);
#endif
...
static int mdm_is_backspace_char(modem_config *cfg, unsigned char ch_raw) {
  return ch_raw == (unsigned char)cfg->s[S_REG_BS]
    || ch_raw == 0x7f
    || ch_raw == 0x14;
}
...
int mdm_handle_char(modem_config *cfg, unsigned char ch) {
  ...
  if(cfg->is_cmd_started == TRUE) { // we previously got an 'AT'
    if(mdm_is_backspace_char(cfg, (unsigned char)ch_raw)) {
      if(cfg->cur_line_idx > 0) {
        cfg->cur_line_idx--;
#ifdef BMC64_DEBUG_PROFILE
        printf("tcpserdbg: command backspace 0x%02x idx %d\r\n",
               (unsigned char)ch_raw, cfg->cur_line_idx);
#endif
      } else if(cfg->is_echo == TRUE) {
