                                    ...
                                    unsigned int *fb_pitch)
{
    if (canvas->video_draw_buffer_callback) {
        int rc = canvas->video_draw_buffer_callback->draw_buffer_alloc(canvas, &canvas->draw_buffer->draw_buffer, fb_width, fb_height, fb_pitch);
        if (rc == 0) {
            canvas->draw_buffer->draw_buffer_non_padded[0] = canvas->draw_buffer->draw_buffer;
            canvas->draw_buffer->draw_buffer_non_padded[1] = canvas->draw_buffer->draw_buffer;
...
static void raster_draw_buffer_free(video_canvas_t *canvas)
{
    if (canvas->video_draw_buffer_callback) {
        ...
        canvas->draw_buffer->draw_buffer_non_padded[0] = NULL;
        canvas->draw_buffer->draw_buffer_non_padded[1] = NULL;
        return;
    }
