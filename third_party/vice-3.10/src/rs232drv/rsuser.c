...
static void rsuser_dbg_log_state(const char *tag)
{
    BMC64_RS232_DEBUG("userport %s fd %d enabled %d dev %d baud %d char_ticks %d "
                      "bit_ticks %d up9600 %d dtr %d rts %d inv r%d c%d s%d "
                      "dcd%d dtr%d ri%d clk %lu",
                      tag, fd, rsuser_enabled, rsuser_device, rsuser_baudrate,
                      char_clk_ticks, bit_clk_ticks, rsuser_up9600,
                      dtr ? 1 : 0, rts ? 1 : 0, rsuser_rtsinv, rsuser_ctsinv,
                      rsuser_dsrinv, rsuser_dcdinv, rsuser_dtrinv,
                      rsuser_riinv, RSUSER_DBG_CLK(maincpu_clk));
}

static void rsuser_dbg_log_tx_edge(uint8_t bit)
{
    static CLOCK last_clk = 0;
    static int last_bit = -1;
    CLOCK delta = last_clk ? maincpu_clk - last_clk : 0;
    unsigned long inferred_baud = delta ? (unsigned long)(cycles_per_sec / delta) : 0;

    if (last_bit != (int)bit) {
        rsuser_dbg_edge_count++;
        if (rsuser_dbg_edge_count <= 64 || (rsuser_dbg_edge_count % 128) == 0) {
            BMC64_RS232_TRACE("userport txedge #%u bit %u prev %d delta %lu "
                              "inferred_baud %lu configured_baud %d bit_ticks %d "
                              "clk %lu clk_start_tx %lu clk_end_tx %lu fd %d",
                              rsuser_dbg_edge_count, bit ? 1U : 0U, last_bit,
                              RSUSER_DBG_CLK(delta), inferred_baud,
                              rsuser_baudrate, bit_clk_ticks,
                              RSUSER_DBG_CLK(maincpu_clk),
                              RSUSER_DBG_CLK(clk_start_tx),
                              RSUSER_DBG_CLK(clk_end_tx), fd);
        }
        last_bit = bit ? 1 : 0;
...
static void rsuser_dbg_print_frame(const char *tag, unsigned int frame_buf, unsigned int frame_valid)
{
    ...
    if (frame_valid > 16) {
        BMC64_RS232_TRACE("userport %s frame valid %u exceeds debug mask range buf 0x%08x",
                          tag, frame_valid, frame_buf);
        return;
    }

    printf("rs232trace: userport %s frame bits oldest-newest ", tag);
    for (i = (int)frame_valid - 1; i >= (int)frame_valid - 10; i--) {
        printf("%u", (frame_buf & masks[i]) ? 1U : 0U);
    }
    printf("\r\n");
#else
    (void)tag;
...
static void rsuser_write_ctrl(uint8_t status, int pulse)
{
...
#ifdef BMC64_DEBUG_PROFILE
    {
        ...
        if (status != dbg_oldstatus) {
            BMC64_RS232_DEBUG("userport ctrl status 0x%02x new_dtr %d new_rts %d "
                              "new_dsr %d fd %d enabled %d clk %lu",
                              status, new_dtr ? 1 : 0, new_rts ? 1 : 0,
                              new_dsr ? 1 : 0, fd, rsuser_enabled,
                              RSUSER_DBG_CLK(maincpu_clk));
            dbg_oldstatus = status;
        }
    }
#endif
        ...
        if (!(buf & masks[valid - 10])) {
            ...
            if (rsuser_dbg_mismatch_count <= 32 || (rsuser_dbg_mismatch_count % 64) == 0) {
                BMC64_RS232_DEBUG("userport mismatch #%u fd %d dev %d baud %d valid %u "
                                  "buf 0x%08x char_ticks %d bit_ticks %d "
                                  "start_bit %u stop_bit %u clk_start_tx %lu "
                                  "clk_start_bit %lu clk_end_tx %lu clk %lu "
                                  "dtr %d rts %d up9600 %d",
                                  rsuser_dbg_mismatch_count, fd, rsuser_device,
                                  rsuser_baudrate, valid, buf, char_clk_ticks,
                                  bit_clk_ticks,
                                  (buf & masks[valid - 1]) ? 1U : 0U,
                                  (buf & masks[valid - 10]) ? 1U : 0U,
                                  RSUSER_DBG_CLK(clk_start_tx),
                                  RSUSER_DBG_CLK(clk_start_bit),
                                  RSUSER_DBG_CLK(clk_end_tx),
                                  RSUSER_DBG_CLK(maincpu_clk),
                                  dtr ? 1 : 0, rts ? 1 : 0, rsuser_up9600);
                rsuser_dbg_print_frame("mismatch", buf, valid);
            }
#endif
        } else {
            c = (buf >> (valid - 9)) & 0xff;
            if (fd >= 0) {
                ...
                if (rsuser_dbg_tx_count <= 32 || (rsuser_dbg_tx_count % 64) == 0) {
                    BMC64_RS232_TRACE("userport tx #%u raw 0x%02x data 0x%02x '%c' "
                                      "fd %d baud %d valid %u buf 0x%08x",
                                      rsuser_dbg_tx_count, c, code[c],
                                      (code[c] >= 32 && code[c] < 127) ? code[c] : '.',
                                      fd, rsuser_baudrate, valid, buf);
                    rsuser_dbg_print_frame("tx", buf, valid);
                }
#endif
