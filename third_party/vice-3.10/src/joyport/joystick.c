...
static bool is_factory_joymap_file(const char *filename)
{
    return filename != NULL && joymap_factory != NULL
        && strcmp(filename, joymap_factory) == 0;
}
...
int joy_arch_mapping_load(const char *filename)
{
    ...
    if (fp == NULL) {
        if (is_factory_joymap_file(filename)) {
            log_message(joy_log,
                        "using minimal default mapping; default joymap `%s' not found.",
                        filename);
            return 0;
        }
...
int joystick_init(void)
{
    ...
    if (res < 0) {
        log_message(joy_log, "using minimal default mapping.");
    }
